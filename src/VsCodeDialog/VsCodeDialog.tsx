...
/**
 ...
 title: 'VS Code not detected',
                ...
                callback: cancelled =>
                    dispatch(
                        cancelCheckCallback(cancelled, showVsCodeDialog(false))
                    ),
...
/**
 title: 'VS Code not detected',
                content: `While the toolchain is installing, we recommend that you [install VS Code](${installLink()}).${
                    process.platform === 'darwin'
                        ? '\n\nOn macOS please make sure that you also follow the instructions for [Launching from the command line](https://code.visualstudio.com/docs/setup/mac#_launching-from-the-command-line).'
                        : ''
                }`,
                callback: cancelled => {
                    if (!cancelled) dispatch(showVsCodeDialog(true));
                },
                confirmLabel: 'I installed VS Code',
                cancelLabel: 'Close',
                optionalLabel: 'Do not remind me of this again',
                onOptional: () =>
...
/**
 ...
 title: 'Missing VS Code extensions',
                ...
                cancelLabel: 'Cancel',
                onOptional: !installingToolchain
                    ? cancelled => {
                          if (!cancelled) openVsCode();
                      }
                    : undefined,
                optionalLabel: !installingToolchain ? 'Open anyway' : '',

                */

/**
                 title: succeded
                ? 'Sucessfully installed VS Code extensions'
